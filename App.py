...
# ─── إعداد الصفحة ────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="توقّع أدائك الدراسي",
    page_icon="🎓",
    layout="centered",
)
...
if st.button("🔮 توقّع أدائي الآن!", use_container_width=True, type="primary"):
    ...
    feature_names = {
        ...
        "extracurricular":"الأنشطة الخارجية","parent_support":"دعم الأهل","screen_time":"وقت الشاشة",
    }
    importance_df = pd.DataFrame({
        "العامل":  list(feature_names.values()),
        "الأهمية": model.feature_importances_,
    }).sort_values("الأهمية", ascending=True)
    st.bar_chart(importance_df.set_index("العامل"))
